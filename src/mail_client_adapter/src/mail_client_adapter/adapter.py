# src/mail_client_adapter/src/mail_client_adapter/adapter.py
"""
Adapter module that connects the service client with the backend logic.

This module acts as a bridge between the Mail Client Service (FastAPI)
and the generated OpenAPI client (mail_client_service_client). It provides
a higher-level abstraction for sending, reading, and managing emails.
"""

from __future__ import annotations

import logging
import sys
from pathlib import Path
from typing import Any

from email_api import (
    Client as EmailApiClient,
    Email,
)

from .mapping import to_email

logger = logging.getLogger(__name__)

# ---------------------------------------------------------------------------
# Import generated client (robust to monorepo layout during local dev)
# ---------------------------------------------------------------------------
try:
    # Preferred: installed package
    from mail_client_service_client import Client as GeneratedClient
    from mail_client_service_client.api.messages import (
        delete_message_messages_message_id_delete as api_delete_message,
        get_message_messages_message_id_get as api_get_message,
        list_messages_messages_get as api_list_messages,
        mark_as_read_messages_message_id_mark_as_read_post as api_mark_as_read,
    )
    logger.debug("Imported generated client from installed package.")
except ModuleNotFoundError:  # pragma: no cover  (environment-specific import path)
    # Fallback: add "<repo-root>/clients/python" to sys.path
    here = Path(__file__).resolve().parent
    repo_root = here.parents[3]
    clients_python = repo_root / "clients" / "python"
    clients_str = str(clients_python)
    if clients_str not in sys.path:
        sys.path.insert(0, clients_str)
    from mail_client_service_client import Client as GeneratedClient
    from mail_client_service_client.api.messages import (
        delete_message_messages_message_id_delete as api_delete_message,
        get_message_messages_message_id_get as api_get_message,
        list_messages_messages_get as api_list_messages,
        mark_as_read_messages_message_id_mark_as_read_post as api_mark_as_read,
    )
    logger.debug("Imported generated client via monorepo fallback path.")

# ---------------------------------------------------------------------------
# Utility: normalize generated objects into dicts/lists with a single return
# ---------------------------------------------------------------------------
def _to_plain(obj: Any) -> Any:
    if obj is None or isinstance(obj, (str, int, float, bool)):
        return obj
    if isinstance(obj, list):
        return [_to_plain(x) for x in obj]
    if isinstance(obj, dict):
        return obj
    if hasattr(obj, "to_dict"):
        try:
            return obj.to_dict()
        except Exception:  # pragma: no cover - defensive
            return obj
    return obj


# ---------------------------------------------------------------------------
# Adapter around generated client
# ---------------------------------------------------------------------------
class ServiceAdapter:
    """Wraps the generated Mail Client Service client."""

    def __init__(self, base_url: str = "http://localhost:8000") -> None:
        self._client = GeneratedClient(base_url=base_url)
        logger.debug("ServiceAdapter initialized with base_url=%s", base_url)

    def list_messages(self, limit: int | None = None) -> list[dict[str, Any]]:
        logger.debug("Adapter.list_messages(limit=%s)", limit)
        data = (
            api_list_messages.sync(client=self._client)
            if limit is None
            else api_list_messages.sync(client=self._client, limit=limit)
        )
        out = _to_plain(data)
        return out if isinstance(out, list) else []

    def get_message(self, message_id: str) -> dict[str, Any]:
        logger.debug("Adapter.get_message(id=%s)", message_id)
        data = api_get_message.sync(client=self._client, message_id=message_id)
        out = _to_plain(data)
        return out if isinstance(out, dict) else {"id": message_id}

    def delete_message(self, message_id: str) -> dict[str, Any]:
        logger.debug("Adapter.delete_message(id=%s)", message_id)
        data = api_delete_message.sync(client=self._client, message_id=message_id)
        out = _to_plain(data)
        return out if isinstance(out, dict) else {"id": message_id, "deleted": True}

    def mark_as_read(self, message_id: str) -> dict[str, Any]:
        logger.debug("Adapter.mark_as_read(id=%s)", message_id)
        data = api_mark_as_read.sync(client=self._client, message_id=message_id)
        out = _to_plain(data)
        return out if isinstance(out, dict) else {"id": message_id, "status": "read"}


# ---------------------------------------------------------------------------
# Thin shim implementing email_api.Client (tests inject a fake adapter here)
# ---------------------------------------------------------------------------
class AdapterClient(EmailApiClient):
    """
    Implements the email_api.Client contract by delegating to a ServiceAdapter.

    Tests may inject a fake by calling:
        AdapterClient(adapter=_FakeAdapter())
    so the constructor must accept an `adapter` keyword argument.
    """

    def __init__(self, adapter: ServiceAdapter | None = None) -> None:
        self._adapter: ServiceAdapter = adapter or ServiceAdapter()

    # ---- email_api.Client methods expected by tests ----

    # The email_api.Client used by tests names this method `list_emails()`.
    def list_emails(self) -> list[Email]:
        raw = self._adapter.list_messages(limit=None)
        return [to_email(m) for m in raw]

    # Kept for parity; some code may call list_messages(limit=...)
    def list_messages(self, limit: int | None = None) -> list[Email]:
        raw = self._adapter.list_messages(limit=limit)
        return [to_email(m) for m in raw]

    def get_message(self, message_id: str) -> Email:
        raw = self._adapter.get_message(message_id)
        return to_email(raw)

    def mark_as_read(self, message_id: str) -> None:
        self._adapter.mark_as_read(message_id)

    def delete_message(self, message_id: str) -> None:
        self._adapter.delete_message(message_id)

    # ---- Contract helper expected by tests: index-based getter ----
    def get_email(self, index: int) -> Email | None:
        """
        Match the ABC contract used in tests: return the Email at `index`
        from `list_emails()`, or None if out-of-bounds.
        """
        emails = self.list_emails()
        if 0 <= index < len(emails):
            return emails[index]
        return None


# ---------------------------------------------------------------------------
# Optional façade kept for unit tests that import it
# ---------------------------------------------------------------------------
class ServiceBackedClient:
    """Simplified façade over ServiceAdapter."""

    def __init__(self, base_url: str = "http://localhost:8000") -> None:
        self.adapter = ServiceAdapter(base_url=base_url)
        logger.debug("ServiceBackedClient initialized with base_url=%s", base_url)

    def fetch_all_emails(self, limit: int | None = None) -> list[dict[str, Any]]:
        return self.adapter.list_messages(limit=limit)

    def fetch_email(self, message_id: str) -> dict[str, Any]:
        return self.adapter.get_message(message_id)

    def remove_email(self, message_id: str) -> dict[str, Any]:
        return self.adapter.delete_message(message_id)

    def mark_email_as_read(self, message_id: str) -> dict[str, Any]:
        return self.adapter.mark_as_read(message_id)